...
<template>
  ...
  >
    <KBUITypography tag="h3" size="h24" weight="bold" class-name="text-center">
      {{ props.title }}
    </KBUITypography>
    ...
    <div class="mt-1 flex items-start gap-1">
      <KBUITypography size="b12">대표자명:</KBUITypography>
      <KBUITypography size="b12">{{ props.ceo }}</KBUITypography>
    </div>
    <div class="mt-1 flex items-start gap-1">
      <KBUITypography size="b12">주소:</KBUITypography>
      <KBUITypography size="b12" :ellipsis="2">{{ props.address }}</KBUITypography>
    </div>
    ...
    <div v-if="props.goods">
      ...
      <div class="grid grid-cols-12">
        <div class="col-span-5 text-start"><KBUITypography size="b14">명칭</KBUITypography></div>
        <div class="col-span-2 text-start"><KBUITypography size="b14">단가</KBUITypography></div>
        <div class="col-span-2 text-center"><KBUITypography size="b14">수량</KBUITypography></div>
        <div class="col-span-3 text-end"><KBUITypography size="b14">금액</KBUITypography></div>
      </div>
      <hr class="my-1 border-b-0.5 border-gray-4" />
      <!-- 영수증 내용 -->
      <ul>
        <li v-for="(item, index) in props.goods" :key="index" class="grid grid-cols-12">
          <div class="col-span-5 text-start">
            <KBUITypography size="b12" class-name="truncate">{{ item.name }}</KBUITypography>
          </div>
          <div class="col-span-2 text-start">
            <KBUITypography size="b12">{{ item.price.toLocaleString() }}</KBUITypography>
          </div>
          <div class="col-span-2 text-center">
            <KBUITypography size="b12">{{ item.quantity }}</KBUITypography>
          </div>
          <div class="col-span-3 text-end">
            <KBUITypography size="b12">{{ (item.price * item.quantity).toLocaleString() }}</KBUITypography>
          </div>
        </li>
      </ul>
    </div>
    ...
    <div class="flex items-start gap-1">
      ...
      <KBUITypography size="b12">{{ dayjs(props.createdAt).format("YYYY.MM.DD HH:mm:ss") }}</KBUITypography>
    </div>
    ...
    >
      ...
      <div v-else class="flex flex-col gap-2 items-center justify-center">
        <KBUITypography size="b20" weight="bold" class-name="text-center" color="red-2">
          경비처리 반려
        </KBUITypography>
