...
const { mutate: postLoginApi } = useMutation({
  ...
  onSuccess: (res) => {
    ...
    document.cookie = `kb_refit_pos_access_token=${res.accessToken}; path=/; secure`;
    getMeApi();
  },
  onError: (error) => {
...
const { mutate: getMeApi } = useMutation({
  ...
  onSuccess: (data) => {
    if ([UserRole.BUSINESS, UserRole.ADMIN].includes(data.role as UserRole)) {
      router.replace("/");
    } else {
      removeTokens();
      toast.add({
        title: "로그인 실패",
        description: "사장님만 접근할 수 있습니다.",
        color: "error",
        duration: 3000,
      });
    }
